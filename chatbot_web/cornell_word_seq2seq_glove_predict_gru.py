...
class CornellWordGloveChatBotGRU(object):
    model = None
    encoder_model = None
    decoder_model = None
    ...
    def __init__(self, type):
        ...
        if(self.type == 'bidirectional'):
            ...
            decoder_state_inputs = [Input(shape=(HIDDEN_UNITS*2,))]
        else:
            # decoder_state_inputs = [Input(shape=(HIDDEN_UNITS,)), Input(shape=(HIDDEN_UNITS,))]
            decoder_state_inputs = [Input(shape=(HIDDEN_UNITS,))]

        # decoder_outputs, state_h, state_c = decoder_gru(decoder_inputs, initial_state=decoder_state_inputs)
        # decoder_outputs, decoder_states = decoder_gru(decoder_inputs, initial_state=decoder_state_inputs)
        decoder_outputs = decoder_gru(decoder_inputs, initial_state=decoder_state_inputs)

        # decoder_states = [state_h, state_c]
        decoder_outputs = decoder_dense(decoder_outputs)

        # self.decoder_model = Model([decoder_inputs] + decoder_state_inputs, [decoder_outputs] + decoder_state)
        self.decoder_model = Model([decoder_inputs] + decoder_state_inputs, [decoder_outputs])
        # self.decoder_model = Model([decoder_inputs] + decoder_state_inputs, [decoder_outputs] + decoder_state_inputs)
...
def main():
    model = CornellWordGloveChatBotGRU()
    model.test_run()
