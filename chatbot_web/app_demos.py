...
# sys.argv[1] -> tipe model
# sys.argv[2] -> buka web ato ngeBLEU

if('char' in os.getenv(sys.argv[1]).lower()):
    # print(os.getenv(sys.argv[1]).lower())
    # sys.exit()
    if('bi' in os.getenv(sys.argv[1])):
        cornell_char_chat_bot = CornellCharChatBot('bidirectional', sys.argv[1])
    else:
        cornell_char_chat_bot = CornellCharChatBot('unidirectional', sys.argv[1])
else:
    # print(os.getenv(sys.argv[1]).lower())
    # sys.exit()
    if('bi' in os.getenv(sys.argv[1])):
        cornell_word_glove_chat_bot = CornellWordGloveChatBot('bidirectional', sys.argv[1])
    else:
        cornell_word_glove_chat_bot = CornellWordGloveChatBot('unidirectional', sys.argv[1])
...
def get_inputs_and_references(dataset_folder_name, sample_amount):
    path = "chatbot_train/data/" + dataset_folder_name + "/test-rev.txt"
...
def bleu_score(dialogs, sample_amount):
    ...
    # with open('bleu/' + os.getenv(sys.argv[1]) + '-' + str(sample_amount) + '.txt', 'w', encoding="latin1") as out_file:
...
def printTestToCSV(dialogs, sample_amount, file_name):
    import pandas as pd
    ...
    test_result['input'] = input_list
    test_result['reference'] = reference_list
    test_result['output'] = output_list
...
if __name__ == '__main__':
    ...
    if(sys.argv[2] == 'web'):
        ...
        if('char' in os.getenv(sys.argv[1]).lower()):
            bleu_score_char('cornell', 1000)
        else:
            bleu_score('cornell', 1000)
    elif(sys.argv[2] == 'tocsv'):
        printTestToCSV('cornell', int(sys.argv[3]), sys.argv[1] + '_test-rev_result_' + sys.argv[3] + '.csv')
