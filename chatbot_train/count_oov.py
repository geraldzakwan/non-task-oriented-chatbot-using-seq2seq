...
for input_words, target_words in zip(input_texts, target_texts):
    ...
    for w in input_words:
        ...
        if w in word2em:
            ...
            if(w not in oov_dict):
                oov_dict[w] = True
                oov_flag = True
        encoder_input_wids.append(emb)

    if(oov_flag):
        utter_with_oov = utter_with_oov + 1

    input_texts_word2em.append(encoder_input_wids)
    encoder_max_seq_length = max(len(encoder_input_wids), encoder_max_seq_length)
    decoder_max_seq_length = max(len(target_words), decoder_max_seq_length)

print("OOV : " + str(len(oov_dict)))
print("wolverine" in oov_dict)
print("Total : " + str(len(word_dict)))
print("wolverine" in word_dict)
print("Utterance with oov " + str(utter_with_oov))

with open(sys.argv[5], 'w', encoding="latin1") as out_file:
    for key in oov_dict:
        out_file.write(key)
        out_file.write('\n')
    out_file.write("Jumlah oov : " + str(len(oov_dict)))
    out_file.write('\n')
    out_file.write("Jumlah vocab : " + str(len(word_dict)))
    out_file.write('\n')
    out_file.write("Jumlah utter dengan oov : " + str(utter_with_oov))
    out_file.write('\n')
sys.exit()
