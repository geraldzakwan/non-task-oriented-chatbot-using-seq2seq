...
if(sys.argv[1] == 'bidirectional'):
    ...
    encoder_outputs, encoder_state_forward_h, encoder_state_forward_c,
    encoder_state_backward_h, encoder_state_backward_c = encoder_lstm(encoder_inputs)
...
decoder_inputs = Input(shape=(None, GLOVE_EMBEDDING_SIZE), name='decoder_inputs', dtype='float32')

if(sys.argv[1] == 'bidirectional'):
    # PAY ATTENTION THAT DECODER AND ENCODER STATE MUST ALWAYS HAVE THE SAME DIMENSION
    # SO, MAKE DECODER UNIT NUMBER TWICE AS USUAL IF BIDIRECTIONAL

    if('attention' in sys.argv[1]):
        decoder_outputs, decoder_state_h, decoder_state_c = AttentionDecoder(HIDDEN_UNITS * 2,
                                                                GLOVE_EMBEDDING_SIZE)(decoder_inputs)
    else:
    ...
    if('attention' in sys.argv[1]):
        decoder_outputs, decoder_state_h, decoder_state_c = AttentionDecoder(HIDDEN_UNITS,
                                                                GLOVE_EMBEDDING_SIZE)(decoder_inputs)
    else:
